...
uint8_t matrix[num_rows + 1 + CC_COUNT];
...
void mergeLeft(half_packet_t packet, int pipe) {
...
void mergeRight(half_packet_t packet, int pipe) {
  uint32_t half = packet.state;
...
void requestEvent() {
  memset(matrix, 0, num_rows + 1 + CC_COUNT);
  ...
  if (per_pipe_length[5] > 0) {
    matrix[num_rows + 1] = per_pipe_data[5][0];
    matrix[num_rows + 2] = per_pipe_data[5][1];
  }

  if (per_pipe_length[6] > 0) {
    matrix[num_rows + 3] = per_pipe_data[6][0];
    matrix[num_rows + 4] = per_pipe_data[6][1];
  }

  Wire.write(matrix, num_rows + 1 + CC_COUNT);
}
